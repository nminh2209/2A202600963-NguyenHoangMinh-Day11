...
def _parse_judge_scores(verdict_text: str) -> dict:
    """Parse multi-criteria judge response into structured scores."""
    ...
    for line in verdict_text.strip().splitlines():
        ...
        for key in scores:
            ...
            if upper.startswith(prefix):
                try:
                    scores[key] = int(upper.split(":", 1)[1].strip().split()[0])
                except (ValueError, IndexError):
                    pass
    verdict = "PASS" if "VERDICT: PASS" in verdict_text.upper() else "FAIL"
